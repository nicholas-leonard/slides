...
-- early-stopping : save the best version of the model on valid set
...
for epoch=1,opt.maxepoch do
   ...
   if validAccuracy > opt.bestAccuracy then
      ...
      print(string.format("New maxima : %f @ %f", opt.bestAccuracy, opt.bestEpoch))
      print(string.format("Saved at %s", opt.savepath))
      wait = 0
   else
      wait = wait + 1
...
--[[
Exercise 2 (10 min) : 
  1. modify the above to take options from the cmd-line (hint : torch.CmdLine())
  2. add Dropout or BatchNormalization between hidden layers (hint : consult nn doc)
